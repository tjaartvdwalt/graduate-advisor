...
* Our idea
  Our approach is to map out the schedule for your entire graduate course using the available data.
...
- courses you would like to take
  ...
  You do not need to specify all courses you want to take, the system will fill any unselected courses so that the course requirements will be fulfilled.
  
* Getting help
** If its slow its working
The backend algorithm does a lot of calculation and may take some time to complete. On our test systems it normally returns within a minute. If it cannot find a valid schedule it should display an error message to that effect.
** Check your configuration
During our testing, we often found found our configuration to be the thing that tripped us up when we couldn't generate a schedule we felt were valid. Often relaxing some restrictions can solve the issue.
