...
function MakeSchedule(selection, semesters, needed, JOi, CJOi) {
    ...
    for(i = 0; i < NEEDED_COURSES; i++) {
        console.log(Order[i]);
        Finished[Order[i]] = PickAppropriate(Order[i], Options, semesters)
...
//rotation object
function PickAppropriate(course_number, Options, semesters) {
    var min_val = 100;
    var min_sem = -1;
    density = [];
    ...
    for(var i = 0; i < Options[course_number].length; i++) {
        if(density[Options[course_number][i].sem - 1] < min_val) {
            min_val = density[Options[course_number][i].sem - 1];
            min_sem = i;
    ...
    return Options[course_number][min_sem];
}
...
//         }
//     }
//     if(pn != undefined && Finished[pn].sem >= Finished[six].sem) {
//         error_type = 1;
//         return false;
//     }
//     return true;
